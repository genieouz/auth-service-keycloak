...
export class NotificationService {
  ...
  constructor(private configService: ConfigService) {
    this.apiUrl = this.configService.get(
      'TOPATOKO_API_URL',
      'https://senegalservices.notification.api.sandbox.topatoko.com'
    );
    this.apiKey = this.configService.get('TOPATOKO_API_KEY', '');
  ...
  async sendSms(smsData: SendSmsRequest): Promise<NotificationResponse> {
    try {
      ...
      const response = await this.httpClient.post('/api/sms/send', {
        to: smsData.to,
        message: smsData.message,
        from: smsData.from || 'SenegalServices',
      });
  ...
  async sendEmail(emailData: SendEmailRequest): Promise<NotificationResponse> {
    try {
      ...
      const response = await this.httpClient.post('/api/email/send', {
        ...
        from: emailData.from || 'noreply@senegalservices.com',
      });
